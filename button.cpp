...
Button::Button(ClutterActor *stage, int width, int height, int x, int y, ClutterColor button1Color) {
	printf("Building events class\n");

    buttonActor = clutter_actor_new();
    // ClutterColor button1Color = { 0, 255, 47, 0xFF };
    clutter_actor_set_background_color (buttonActor, &button1Color);
    ...
    g_signal_connect(buttonActor, "touch-event", G_CALLBACK (handleEvents), buttonActor);
    g_signal_connect(buttonActor, "button-press-event", G_CALLBACK (handleEvents), buttonActor);
    g_signal_connect(buttonActor, "button-release-event", G_CALLBACK (handleEvents), buttonActor);
}
...
gboolean Button::handleEvents (ClutterActor *actor,
    ...
    ClutterActor *button = CLUTTER_ACTOR (user_data);
